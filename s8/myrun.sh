...
if [ $# != 1 ];then
	echo "";
	echo "Haven't given step indicator!"
	echo "";
...
. ./cmd.sh ## You'll want to change cmd.sh to something that will work on your system.
	...
	echo "./cmd.sh should be ended.";
	echo "";
	echo "call to utils/parse_options.sh";
. utils/parse_options.sh  # e.g. this parses the --stage option if supplied.
	echo "utils/parse_options.sh should be ended.";
...
#wsj1=/export/corpora5/LDC/LDC94S13B
	...
	echo "";
	echo "setting wsj corpus";
wsj0=/projekte/slu/Data/WSJ0
wsj1=/projekte/slu/Data/WSJ1
	echo "wsj corpus have been set to $wsj0 and $wsj1";
	echo "";
...
case $step in
  ...
  step0)
    echo "$step begins";
    echo "Now please step in to step0.x incrementally.";	
	;;
	step0.1)
	echo "";
	echo "$step begins";
...
if [ $stage -le 0 ]; then
  ...
  echo "just done: local/wsj_data_prep.sh $wsj0/??-{?,??}.? $wsj1/??-{?,??}.?  || exit 1;";
fi
...
if [ $stage -le 0 ]; then
  ...
  echo "just done: local/wsj_prepare_dict.sh --dict-suffix "_nosp" || exit 1;";
fi
...
if [ $stage -le 0 ]; then
  ...
  echo "just done: utils/prepare_lang.sh data/local/dict_nosp \ ";
  echo "                      "<SPOKEN_NOISE>" data/local/lang_tmp_nosp data/lang_nosp || exit 1;";
fi
...
if [ $stage -le 0 ]; then
  ...
  echo "just done: local/wsj_format_data.sh --lang-suffix "_nosp" || exit 1;";
fi
